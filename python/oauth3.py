...
def AccountsUrl(command):
  ...
  """
  return '{}/{}'.format(GOOGLE_ACCOUNTS_BASE_URL, command)
...
def FormatUrlParams(params):
  ...
  for param in sorted(params.items(), key=lambda x: x[0]):
    param_fragments.append('{}={}'.format(param[0], UrlEscape(param[1])))
...
def GeneratePermissionUrl(client_id, scope='https://mail.google.com/'):
  ...
  return '{}?{}'.format(AccountsUrl('o/oauth2/auth'),
...
def main(argv):
  ...
  if options.refresh_token:
    ...
    print (GeneratePermissionUrl(options.client_id, options.scope))
