...
def AuthorizeTokens(client_id, client_secret, authorization_code):
    ...
    response = url.urlopen(request_url, urllib.parse.urlencode(params).encode('utf-8')).read()
...
def RefreshToken(client_id, client_secret, refresh_token):
    ...
    response = urllib.urlopen(request_url, urllib.parse.urlencode(params)).read()
    return json.loads(response)
